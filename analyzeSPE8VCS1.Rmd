...
Fo2 <- s.gt %>% 
  single_raincloud_plot(.$M, 200, 1400, 200, "Label-Matching RT (ms)") + 
  geom_hline(yintercept=thresholds$Outlier, linetype="dotted") +
  geom_hline(yintercept=thresholds$Extreme, linetype='dashed', color='red', linewidth=0.5)
...
In the left plot, Self vs. Stranger refers to the label. In the nonmatch trials, the label and the color referred to different persons; thus, the trials were classified based on the label. Following [Wade & Vickery (2017)](https://link.springer.com/article/10.3758/s13414-017-1307-8), congruency benefit was computed by comparing responses to match versus nonmatch trials for each label, thereby controlling for label processing speed. The right plot displays these values.
We defined self-bias as the difference between the Self and Stranger conditions in the right plot.
...
As with accuracy, we defined self-bias as the difference between the Self and Stranger conditions in the right plot.
...
As described above, self-bias was calculated from the congruency benefits of accuracy and RT [(Wade & Vickery, 2017)](https://link.springer.com/article/10.3758/s13414-017-1307-8).
...
tmp %>% 
  kable(digits = c(2,2,0,0), 
        format = "simple", caption = "Self-bias @ label-matching judgment")
```
...
In the right plot, the Y-axis represents self-bias. The average of the First and Second conditions was defined as self-bias in the WM task.
...
Self-bias was defined as the average of the First and Second conditions.
